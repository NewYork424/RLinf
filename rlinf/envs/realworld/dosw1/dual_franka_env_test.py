...
# Robotiq raw position range for continuous gripper mode
_GRIPPER_RAW_OPEN = 0
_GRIPPER_RAW_CLOSED = 255
_GRIPPER_RAW_MIN_DELTA = 2
...
class DualFrankaRobotConfig:
    ...
    continuous_gripper: bool = False  # true = continuous position tracking, false = binary open/close
...
class DualFrankaEnv(gym.Env):
    ...
    def __init__(
        ...
        self._last_gripper_raw_positions: list[int | None] = [None, None]
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self._num_steps = 0
        self._success_hold_counter = 0
        self._last_gripper_raw_positions = [None, None]
    ...
    def _gripper_action(self, arm: int, ctrl, state, position: float) -> bool:
        if self.config.continuous_gripper:
            # Map [-1, 1] → Robotiq raw [0, 255]: +1 = fully open, -1 = fully closed
            clipped = float(np.clip(position, -1.0, 1.0))
            raw_position = int(round(_GRIPPER_RAW_OPEN + (1.0 - clipped) * 0.5 * (_GRIPPER_RAW_CLOSED - _GRIPPER_RAW_OPEN)))
            last = self._last_gripper_raw_positions[arm]
            if last is not None and abs(raw_position - last) < _GRIPPER_RAW_MIN_DELTA:
                return False
            ctrl.move_gripper(raw_position)
            self._last_gripper_raw_positions[arm] = raw_position
            return True
