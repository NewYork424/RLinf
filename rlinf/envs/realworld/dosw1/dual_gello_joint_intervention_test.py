...
class DualGelloJointIntervention(gym.ActionWrapper):
    def __init__(
        ...
        self._aligned = False
        # For continuous gripper mode: track last gripper action to detect change
        self._last_gripper_actions: np.ndarray | None = None
        self._gripper_action_threshold = 0.02
    ...
    def _stream_loop(self) -> None:
        ...
        while self._stream_running:
            ...
            if self.gripper_enabled:
                # Check continuous_gripper mode: if true, gripper handled by env.step only
                # If false, use original binary edge-triggered logic
                try:
                    continuous = self.get_wrapper_attr("config").continuous_gripper
    ...
    def action(self, action: np.ndarray) -> tuple[np.ndarray, bool]:
        ...
        if self.gripper_enabled:
            ...
            for grip in (left_g, right_g):
                ...
                g = np.clip(g, -1.0, 1.0)
                grippers.append(g)
            # Check continuous_gripper mode for gripper_active detection
            try:
                continuous = self.get_wrapper_attr("config").continuous_gripper
            except AttributeError:
                continuous = False
            if continuous:
                # Change-tracking mode: only active if value changed > threshold
                gripper_values = np.array([float(g.item()) for g in grippers], dtype=np.float32)
                if self._last_gripper_actions is None:
                    gripper_active = True
                elif np.max(np.abs(gripper_values - self._last_gripper_actions)) > self._gripper_action_threshold:
                    gripper_active = True
                if gripper_active:
                    self._last_gripper_actions = gripper_values
            else:
                # Original threshold mode: active if abs > 0.5
                for g in grippers:
