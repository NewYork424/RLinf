...
class DualFrankaEnv(gym.Env):
    ...
    @staticmethod
    def _gripper_action_to_raw_position(position: float) -> int:
        """Map action +1=open, -1=closed to Robotiq raw 0..255."""
        clipped = float(np.clip(position, -1.0, 1.0))
        raw_span = _GRIPPER_RAW_CLOSED - _GRIPPER_RAW_OPEN
        raw = _GRIPPER_RAW_OPEN + (1.0 - clipped) * 0.5 * raw_span
        return int(round(raw))

    def _gripper_action(self, arm: int, ctrl, state, position: float) -> bool:
        del state
        raw_position = self._gripper_action_to_raw_position(position)
        last_position = self._last_gripper_raw_positions[arm]
        if (
            last_position is not None
            and abs(raw_position - last_position) < _GRIPPER_RAW_MIN_DELTA
        ):
            return False

        # Fire-and-forget: blocking gripper RPCs stretch eval steps and ring out j7.
