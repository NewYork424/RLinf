...
class DualGelloJointIntervention(gym.ActionWrapper):
    ...
    def _stream_loop(self) -> None:
        # Gripper commands stay step-gated through env.step; streaming serial
        # writes at joint-tracker frequency would starve the Modbus channel.
    ...
    def action(self, action: np.ndarray) -> tuple[np.ndarray, bool]:
        ...
        if self.gripper_enabled:
            ...
            for grip in (left_g, right_g):
                ...
                g = np.clip(g, -1.0, 1.0)
                grippers.append(g)
                gripper_values.append(float(g.item()))
            gripper_values = np.asarray(gripper_values, dtype=np.float32)
            if self._last_gripper_actions is None:
                gripper_active = True
            elif (
                np.max(np.abs(gripper_values - self._last_gripper_actions))
                > self._gripper_action_threshold
            ):
                gripper_active = True
            if gripper_active:
